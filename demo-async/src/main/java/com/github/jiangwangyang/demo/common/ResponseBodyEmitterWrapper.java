...
@Deprecated
@Slf4j
public class ResponseBodyEmitterWrapper extends ResponseBodyEmitter {
    ...
    public synchronized void send(@NonNull Object object, @Nullable MediaType mediaType) {
        try {
            ...
            log.warn("ResponseBodyEmitterWrapper.send() IOException: {}", e.getMessage());
        } catch (IllegalStateException e) {
            log.warn("ResponseBodyEmitterWrapper.send() IllegalStateException: {}", e.getMessage());
        }
    }
    ...
    public synchronized void complete() {
        try {
            ...
            log.warn("ResponseBodyEmitterWrapper.complete() IllegalStateException: {}", e.getMessage());
        }
    }
